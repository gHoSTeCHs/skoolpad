...
import type { EnumOption, McqConfig, QuestionNode } from '@/types/questions';
...
interface SingleChoiceAuthorProps {
...
export function SingleChoiceAuthor({ question, enumOptions, onDirtyChange }: SingleChoiceAuthorProps) {
    ...
    const setConfig = useCallback((next: McqConfig) => {
    ...
    function setCorrect(idx: number) {
        setConfig({
            ...config,
            options: config.options.map((o, i) => ({ ...o, is_correct: i === idx })),
        });
    }
    ...
    return (
        <form onSubmit={save} className="space-y-5">
            <StemCard
                title="Stem"
                description="Single-correct multiple choice. Click a row to mark it correct."
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Click a row to mark it as correct. Hover to edit text.</CardDescription>
                </CardHeader>
                <CardContent>
                    <div className="space-y-1.5">
                        {config.options.map((option, idx) => {
                            ...
                            return (
                                <div
                                    key={idx}
                                    onClick={() => !isEditing && setCorrect(idx)}
                                    className={
                                ...
                                >
                                    ...
                                    >
                                        {option.is_correct ? '✓' : OPTION_LABELS[idx]}
                                    </span>
